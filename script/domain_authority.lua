-- basic ws
-- use getgenv().signalWs = "URL HERE"
local uri = getgenv().signalWs .. "/local_server.js" or readfile("LDXWebSocket.ldx")
...
player.Chatted:Connect(function(message) --send msg and auto complete stuff
	...
	elseif string.find(msg,"/announcement") or string.find(msg,"/announce") then
		local textan = table.concat(args," ",3) 
		if args[2] == "server" or args[2] == "session" then
			ws:Send("/announcement" .. " " .. game.JobId .. " " .. textan)
		else
			ws:Send("/announcement" .. " " .. args[2] .. " " .. textan)
		end --/server ban someone (but it need owner authority) maybe latter
...
ws.OnMessage:Connect(function(message)
	...
	if not f then
		...
		elseif signal[1] == "/code" or signal[1] == "/run" then
			local func, err = loadstring(table.concat(signal," ",2))
			if func then
				func()
			else
				warn(err)
...
function crgui()
	...
	title.Text = "LIEX Local Host By Jorell"
